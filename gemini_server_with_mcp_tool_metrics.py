...
# This is the key change for observability
Traceloop.init(app_name="gemini-mcp-server-withtool")
...
async def generate_image(prompt: str) -> Image:
    ...
    # Create specific span for Instana Tool Metrics
    # This manual instrumentation adds the specific attributes Instana looks for
    with tracer.start_as_current_span(f"{tool_name}.tool") as span:
        # Set Traceloop/Instana specific attributes
        span.set_attribute("traceloop.span.kind", "tool")
        span.set_attribute("traceloop.entity.name", tool_name)
        span.set_attribute("traceloop.workflow.name", "gemini_mcp_workflow")
...
def start_server() -> None:
    ...
    This function blocks — call it from __main__.
    """
